...
#Run this script in a directory ONLY the .fq HiFi reads. Name this directory the prefix that will signify the sample. 
#This script is identical to BFB_pipeline.sh except dorado alignment has been replaced with minimap2.
#If HiFi reads are in bam format then use samtools bam2fq to convert to a fastq.
...
#MINIMAP REPLACEMENT FOR HiFi
minimap2 -ax map-hifi -t 20 $REFERENCE *.fq | samtools view -b - > ${PREFIX}_aligned.bam
