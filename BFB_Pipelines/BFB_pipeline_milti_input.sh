...
#This is identical to BFB_pipeline.sh except is for samples where multiple sequencing runs exist. 
#This script is identical to BFB_pipeline.sh except multiple dorado alignments are in one directory. Repalce A,B,C.bam with the names of unaligned .bam files. 
#If more than three unaligned files exist add more of the align more alignments can be done with a new file name (not X, Y, Z.bam) and added in the samtools merge
#If the files were already aligned then a new alignment hereis not necessary and old alignments can be copied here and replaced with the X.bam Y.bam Z.bam in samtools merge.
...
echo "Aligning reads..."
####Repeat the alignment with all files in the direcotry.
singularity exec /apps/singularity-images/dorado_1.1.1.sif dorado aligner -t 20 $REFERENCE A.bam > X.bam
singularity exec /apps/singularity-images/dorado_1.1.1.sif dorado aligner -t 20 $REFERENCE B.bam > Y.bam
singularity exec /apps/singularity-images/dorado_1.1.1.sif dorado aligner -t 20 $REFERENCE C.bam > Z.bam


###Replace X,Y,Z with real aligned files here.
samtools merge -o ${PREFIX}_aligned.bam X.bam Y.bam Z.bam

echo "Sorting and indexing global BAM..."
samtools sort -@ 20 -m 2G -o ${PREFIX}_sorted.bam ${PREFIX}_aligned.bam
...
# ---------------------------------------------------------
# 6. Isolate Real Foldback Inversions
# ---------------------------------------------------------
...
bam_file = "${PREFIX}_SNP_filtered.bam"
output_bam = "${PREFIX}_foldbacks.bam"
